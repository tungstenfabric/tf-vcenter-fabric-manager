...
def validate_vnc_port(vnc_port):
    esxi_port_info = vnc_port.get("esxi_port_info")
    if not esxi_port_info:
        raise VNCPortValidationError(
            "No ESXi info could be read from port %s", vnc_port["fq_name"][-1]
        )
    dvs_name = esxi_port_info.get("dvs_name")
    if not dvs_name:
        raise VNCPortValidationError(
            "No DVS name could be read from port %s", vnc_port["fq_name"][-1]
        )
    pi_back_refs = vnc_port.get("physical_interface_back_refs")
    if not pi_back_refs:
        raise VNCPortValidationError(
            "No Physical Interfaces could be found for port %s",
            vnc_port["fq_name"][-1],
        )
...
class PhysicalInterfaceService(Service):
    ...
    def _populate_pr_to_fabric(self):
        ...
        for pr in pr_list:
            fabric_refs = pr.get("fabric_refs")
    ...
    def _get_vnc_nodes(self):
        ...
        vnc_nodes = self._vnc_api_client.get_nodes_by_host_names(
            host_names_in_vcenter, fields=["fq_name", "ports"]
        )
        return vnc_nodes

    def _create_pis_for_node(self, vnc_node):
        node_ports = self._vnc_api_client.get_node_ports(
            vnc_node, fields=["esxi_port_info", "physical_interface_back_refs"]
        )
    ...
    def _create_pis_for_port(self, vnc_node, vnc_port):
        ...
        host_name = vnc_node["fq_name"][-1]
        dvs_name = vnc_port["esxi_port_info"]["dvs_name"]
        vnc_pis = self._vnc_api_client.get_pis_by_port(
            vnc_port, fields=["parent_uuid"]
        )
    ...
    def _create_pi_models_in_db(self, vnc_pis, host_name, dvs_name):
        ...
        for vnc_pi in vnc_pis:
            fabric_uuid = self._pr_to_fabric[vnc_pi.get("parent_uuid")]

...
class VNCAPIClient(object):
    ...
    def read_all_physical_routers(self):
        return self.vnc_lib.physical_routers_list(detail=True)
    ...
    def get_nodes_by_host_names(self, esxi_names):
        ...
        return self.vnc_lib.nodes_list(detail=True, filters=filters)

    def get_node_ports(self, node):
        port_uuids = [port_ref["uuid"] for port_ref in node.get_ports()]
        return self.vnc_lib.ports_list(detail=True, obj_uuids=port_uuids)

    def read_all_ports(self):
        return self.vnc_lib.ports_list(detail=True)
    ...
    def get_pis_by_port(self, port):
        pi_uuids = [
            pi_ref["uuid"]
            for pi_ref in port.get_physical_interface_back_refs() or ()
        ]
        return self.vnc_lib.physical_interfaces_list(
            detail=True, obj_uuids=pi_uuids
        )
